%function [T, Y] = main(matrix, z)

%initCobraToolbox(false);
%pre_model = readCbModel('C:\Users\Bisbii\PythonProjects\ExpAlgae\data\models\model_dfba.xml');

%writeCbModel(pre_model, 'format', 'mat', 'fileName', 'C:\Users\Bisbii\PythonProjects\ExpAlgae\data\models\dsalina_dfba.mat');
...
DB = zeros(nmodel);
for i = 1:nmodel
    load C:\Users\Bisbii\PythonProjects\ExpAlgae\data\models\dsalina_dfba.mat
    models{i} = model;
    DB(i) = 10000;
end
INFO.DB = DB;
...
RxnNames = {biomass, 'PRISM_white_LED__extr', 'EX_C00009__dra', 'EX_C00244__dra', 'DM_C00369__chlo', 'DM_C05306__chlo', 'DM_C05307__chlo', 'DM_C02094__chlo', 'DM_C00422__lip', 'DM_C00116__cytop', ...
                'EX_C00011__dra', 'DM_C00244__cytop'};
exID = cell(nmodel);
for n = 1:nmodel
...
chlorophyl_quota = 0.045; %0.00066;
...
starch_quota = 0.05;
glycerol_quota = 0.15;
carotene_quota = 0.0025; %0.002;
tag_quota = 0.01;
INFO.nacl = 135;
%matrix{end}({sheet_name{z}},{'Salinity g/L'}).("Salinity g/L");
...
hold off;
legend([a; b] , "HPO4", "NO3");
filename = strcat('C:\Users\Bisbii\PythonProjects\ExpAlgae\data\dfba\',strcat(strcat(sheet_name{z}, '_Concentrations'), '.fig'));
savefig(filename);
...
yyaxis right
e = plot(T,Y(:,[7,8]));
ylabel('b-Carotene & TAG (g/L)');
hold off
legend('Total Biomass', 'Functional Biomass', 'Starch', 'Glycerol' , 'Total Biomass (Exp)', 'b-Carotene', 'TAG '); % tag requires a space after because is also an argument for legend in matlab -_-
filename = strcat('C:\Users\Bisbii\PythonProjects\ExpAlgae\data\dfba\', strcat(strcat(sheet_name{z}, '_Biomass'), '.fig'));
savefig(filename);
...
hold off
legend('Starch', 'Glycerol', 'TAG ', 'Chlorophyll', 'Carotene'); % tag requires a space after because is also an argument for legend in matlab -_-
filename = strcat('C:\Users\Bisbii\PythonProjects\ExpAlgae\data\dfba\', strcat(strcat(sheet_name{z}, '_Quotas'), '.fig'));
savefig(filename);
