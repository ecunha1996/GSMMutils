...
function CPLEXobjs = MosekConstruct(model,nmodel,C,ncost,sense,tol,tolPh1,solver,disp,error)
global CPLEXobjs
for k=1:nmodel
    A = model{k}.A;
    b = model{k}.b;
    ...
    CPLEXobjs{k,1}.c = -b;
    CPLEXobjs{k,1}.blx = -Inf*ones(length(b),1);
    CPLEXobjs{k,1}.bux = Inf*ones(length(b),1);
    CPLEXobjs{k,1}.a = sparse(A');
    CPLEXobjs{k,1}.blc = -Inf*ones(size(A,2),1);
    CPLEXobjs{k,1}.buc = C{k}(:,1);
    
    for i=2:ncost(k)
        if(sense{k}(i)==-1)
            C{k}(:,i) = -C{k}(:,i);
        end
        CPLEXobjs{k,i}.c = -1*[b;zeros(i-1,1)];
        CPLEXobjs{k,i}.blx = -Inf*ones(length(b)+i-1,1);
        CPLEXobjs{k,i}.bux = Inf*ones(length(b)+i-1,1);
        CPLEXobjs{k,i}.a = sparse([A;(C{k}(:,1:i-1))']');
        CPLEXobjs{k,i}.blc = -Inf*ones(size(A,2),1);
        CPLEXobjs{k,i}.buc = C{k}(:,i);
    end

    [flux] = MosekLexicographicSolve(k,b,ncost(k),tolPh1,error,0);
    if disp == 1
      for i=2:ncost(k)
