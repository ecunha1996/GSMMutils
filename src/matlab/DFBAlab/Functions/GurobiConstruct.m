...
function CPLEXobjs = GurobiConstruct(model,nmodel,C,ncost,sense,tol,tolPh1,solver,disp,error)
global CPLEXobjs
for k=1:nmodel
    A = model{k}.A;
    b = model{k}.b;
    ...
    CPLEXobjs{k,1}.obj = -b;
    CPLEXobjs{k,1}.lb = -Inf*ones(length(b),1);
    CPLEXobjs{k,1}.ub = Inf*ones(length(b),1);
    CPLEXobjs{k,1}.A = sparse(A');
    CPLEXobjs{k,1}.sense = ['<'];
    CPLEXobjs{k,1}.rhs = C{k}(:,1);
    
    for i=2:ncost(k)
        if(sense{k}(i)==-1)
            C{k}(:,i) = -C{k}(:,i);
        end
        CPLEXobjs{k,i}.obj = -1*[b;zeros(i-1,1)];
        CPLEXobjs{k,i}.lb = -Inf*ones(length(b)+i-1,1);
        CPLEXobjs{k,i}.ub = Inf*ones(length(b)+i-1,1);
        CPLEXobjs{k,i}.A = sparse([A;(C{k}(:,1:i-1))']');
        CPLEXobjs{k,i}.sense = ['<'];
        CPLEXobjs{k,i}.rhs = C{k}(:,i);
    end

    [flux] = GurobiLexicographicSolve(k,b,ncost(k),tolPh1,error,0);
    if disp == 1
      for i=2:ncost(k)
