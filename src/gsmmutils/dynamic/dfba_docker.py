...
class DFBA:
    ...
    def run(self):
        cmd = (f'podman run --name dfba -v {self.data_directory}:/home/data/:Z -v '
               f'{self.src_directory}:/home/src/:Z davidtourigny/dfba bash -c "/usr/bin/python3 -m pip install --upgrade pip && pip install -q --root-user-action=ignore matplotlib seaborn scipy '
