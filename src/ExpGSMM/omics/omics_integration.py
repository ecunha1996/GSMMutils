...
from ExpGSMM.bio.genes import Genes
from ExpGSMM.graphics.plot import clustermap
from ExpGSMM.io import write_specific_models
from ExpGSMM.io import read_csv
from ExpGSMM.utils.utils import run, differential_reaction_capacity
...
class OmicsIntegration:
    ...
    def get_getmm(self, counts_file=None, data_path = None, output_file=None):
        ...
        cmd = ["Rscript", join(os.getcwd(), "../../src/ExpGSMM/omics/GeTMM.R"), counts_file , data_path, output_file, ','.join(self.groups)]
        run(cmd)
    ...
    def get_degs(self, getmm_file=None, output_file=None):
        ...
        cmd = ["Rscript", join(os.getcwd(), "../../src/ExpGSMM/omics/DGE.R"), getmm_file, output_file, ','.join(self.groups)]
        run(cmd)
    ...
    def get_flux_change(self, combine_all=False, method_1=None, condition_1=None, method_2=None, condition_2=None, threshold: float = 0.1):
        from ExpGSMM.utils.utils import flux_change
        if combine_all:
    ...
    def get_reaction_capacity(self, condition, fva_solution):
        from ExpGSMM.utils.utils import reaction_capacity
