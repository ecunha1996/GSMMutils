...
class DFBA:
    ...
    def run(self):
        cmd = (f'podman run --name optimization -v {self.data_directory}:/home/data/:Z -v '
               f'{self.src_directory}:/home/src/:Z davidtourigny/dfba '
               f'sh -c "/usr/bin/python3 -m pip install --upgrade pip && pip install -q --root-user-action=ignore mewpy matplotlib seaborn scipy '
               f'joblib openpyxl psutil plotly kaleido timeout-decorator tqdm parallelbar && '
               f'python3 /home/examples/optimization/optimization.py > /home/data/output.txt"')  #
